...
impl Editor {
    ...
    fn render_wrapped(&self, area: Rect, buf: &mut RatatuiBuffer) {
        ...
        for row in start_row..line_count {
            ...
            if let Some(ln_str) = self.get_line_number_str(row, cursor_pos.row) {
                let ln_style = if is_cursor_line { self.line_number_style.add_modifier(Modifier::BOLD) } else { self.line_number_style };
    ...
    fn get_char_style_fast(&self, row_styles: &[Style], col: usize, row: usize, selection: Option<(Position, Position)>, block_selection: Option<(Position, Position)>) -> Style {
