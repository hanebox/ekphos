...
impl SearchIndex {
    pub fn retained_bytes(&self) -> usize {
        let term_bytes = self
            .terms
            .iter()
            .map(|(term, postings)| term.capacity() + postings.capacity() * std::mem::size_of::<(usize, usize, usize)>())
            .sum::<usize>();
        let line_bytes = self
            .lines
            .iter()
            .map(|lines| lines.capacity() * std::mem::size_of::<String>() + lines.iter().map(String::capacity).sum::<usize>())
            .sum::<usize>();
        let meta_bytes = self
            .file_meta
            .keys()
            .map(|path| path.capacity() + std::mem::size_of::<(u64, usize)>())
            .sum::<usize>();
        term_bytes + line_bytes + meta_bytes + self.notes_dir.capacity()
    }
    ...
    fn index_note(&mut self, note_idx: usize, rel_path: &str, content: &str, mtime: u64) {
        ...
        for (line_num, line) in lines.iter().enumerate() {
            ...
            for word in line.split(|c: char| !c.is_alphanumeric()).filter(|w| (1..=50).contains(&w.chars().count())) {
                ...
                if let Some(char_pos) = find_char_position(&line_chars, &word_lower) {
                    self.terms.entry(word_lower).or_default().push((note_idx, line_num, char_pos));
                }
            }
        }
    ...
    pub fn remove_deleted(&mut self, current_paths: &[String]) {
        ...
        // Find deleted files
        let deleted: Vec<String> = self.file_meta.keys().filter(|p| !current_set.contains(p)).cloned().collect();
