...
impl App {
    pub fn clipboard(&self) -> &dyn crate::clipboard::Clipboard {
    ...
    pub fn next_sidebar_item(&mut self) {
        if self.vault.sidebar_items.is_empty() {
            return;
        }
        self.vault.selected_sidebar_index = (self.vault.selected_sidebar_index + 1) % self.vault.sidebar_items.len();
    ...
    pub fn previous_sidebar_item(&mut self) {
        ...
        self.vault.selected_sidebar_index = if self.vault.selected_sidebar_index == 0 { self.vault.sidebar_items.len() - 1 } else { self.vault.selected_sidebar_index - 1 };
        self.sync_selected_note_from_sidebar();
        self.update_content_items();
        self.update_outline();
    }
    ...
    pub fn goto_first_sidebar_item(&mut self) {
        ...
        self.vault.selected_sidebar_index = 0;
        self.sync_selected_note_from_sidebar();
        self.update_content_items();
        self.update_outline();
    }
    ...
    pub fn goto_last_sidebar_item(&mut self) {
        ...
        self.vault.selected_sidebar_index = self.vault.sidebar_items.len() - 1;
        self.sync_selected_note_from_sidebar();
        self.update_content_items();
        self.update_outline();
    }
    ...
    pub fn handle_sidebar_enter(&mut self) {
        let item_info = self.vault.sidebar_items.get(self.vault.selected_sidebar_index).map(|item| match &item.kind {
    ...
    pub fn toggle_folder(&mut self, path: PathBuf) {
        let new_state = !self.vault.folder_states.get(&path).copied().unwrap_or(false);
        self.vault.folder_states.insert(path.clone(), new_state);
        Self::update_folder_in_tree(&mut self.vault.file_tree, &path, new_state);
    ...
    pub fn toggle_outline_collapsed(&mut self) {
        self.state.outline_collapsed = !self.state.outline_collapsed;
    }

    pub fn is_sidebar_minimized(&self) -> bool {
        self.state.sidebar_collapsed || Config::panel_width_is_minimized(self.state.config.sidebar_width_percent)
    }

    pub fn is_outline_minimized(&self) -> bool {
        self.state.outline_collapsed || Config::panel_width_is_minimized(self.state.config.outline_width_percent)
    }

    pub fn resize_focused_panel(&mut self, delta: i64) {
        if self.state.zen_mode {
            self.state.status_message = Some("Panel resizing is unavailable in zen mode".to_string());
            return;
        }
        let (panel_name, collapsed, current_width) = match self.state.focus {
            Focus::Sidebar => ("Sidebar", self.state.sidebar_collapsed, self.state.config.sidebar_width_percent),
            Focus::Outline => ("Outline", self.state.outline_collapsed, self.state.config.outline_width_percent),
            Focus::Content => {
                self.state.status_message = Some("Focus the sidebar or outline to resize".to_string());
                return;
            }
        };
        if collapsed {
            self.state.status_message = Some(format!("{} is collapsed", panel_name));
            return;
        }
        ...
        if resized_width == effective_width {
            ...
            self.state.status_message = Some(format!("{} width is already at the {} ({}%)", panel_name, boundary, effective_width));
            return;
        }
        match self.state.focus {
            Focus::Sidebar => self.state.config.sidebar_width_percent = resized_width,
            Focus::Outline => self.state.config.outline_width_percent = resized_width,
            Focus::Content => unreachable!(),
        }
        let minimized = if Config::panel_width_is_minimized(resized_width) { " (minimized)" } else { "" };
        self.state.status_message = Some(format!("{} width: {}%{}", panel_name, resized_width, minimized));
    ...
    pub fn toggle_frontmatter_hidden(&mut self) {
        self.document.frontmatter_hidden = !self.document.frontmatter_hidden;
        self.update_content_items();
    }

    pub fn update_filtered_indices(&mut self) {
        if self.search.search_query.is_empty() {
            self.search.search_matched_notes.clear();
            self.search.filtered_indices.clear();
            return;
        }
        let query = self.search.search_query.to_lowercase();
        self.search.search_matched_notes = self.vault.notes.iter().enumerate().filter(|(_, note)| note.title.to_lowercase().contains(&query)).map(|(i, _)| i).collect();
        for &note_index in &self.search.search_matched_notes {
        ...
        self.search.filtered_indices =
            self.vault.sidebar_items.iter().enumerate().filter(|(_, item)| if let SidebarItemKind::Note { note_id } = &item.kind { self.note_index_for_id(*note_id).is_some_and(|index| self.search.search_matched_notes.contains(&index)) } else { false }).map(|(i, _)| i).collect();
        if !self.search.filtered_indices.is_empty() {
            self.vault.selected_sidebar_index = self.search.filtered_indices[0];
            self.sync_selected_note_from_sidebar();
            self.update_content_items();
            self.update_outline();
        }
    ...
    pub fn activate_sidebar_search(&mut self) {
        self.search.pre_search_folder_states = Some(self.vault.folder_states.clone());
        self.search.pre_search_sidebar_index = Some(self.vault.selected_sidebar_index);
        self.search.search_active = true;
    ...
    pub fn clear_search(&mut self) {
        ...
        self.search.filtered_indices.clear();
        self.search.search_matched_notes.clear();
        if let Some(saved_states) = self.search.pre_search_folder_states.take() {
            self.vault.folder_states = saved_states;
            Self::update_tree_expanded_states(&mut self.vault.file_tree, &self.vault.folder_states);
            self.rebuild_sidebar_items();
        }
        if let Some(saved_index) = self.search.pre_search_sidebar_index.take() {
            self.vault.selected_sidebar_index = saved_index.min(self.vault.sidebar_items.len().saturating_sub(1));
        }
    }
    ...
    pub fn start_buffer_search_with_direction(&mut self, direction: SearchDirection) {
        self.search.buffer_search.active = true;
        self.search.buffer_search.query.clear();
        self.search.buffer_search.matches.clear();
        self.search.buffer_search.current_match_index = 0;
        self.search.buffer_search.direction = direction;
    }

    pub fn end_buffer_search(&mut self) {
        self.search.buffer_search.clear();
    }

    pub fn perform_buffer_search(&mut self) {
        self.search.buffer_search.matches.clear();
        self.search.buffer_search.current_match_index = 0;
        if self.search.buffer_search.query.is_empty() {
            return;
        }
        let query = if self.search.buffer_search.case_sensitive { self.search.buffer_search.query.clone() } else { self.search.buffer_search.query.to_lowercase() };
        let matches = if self.editor.mode == Mode::Edit {
            let snapshot = self.editor.snapshot();
            find_buffer_matches(snapshot.iter_lines(), &query, self.search.buffer_search.case_sensitive)
        } else if let Some(body) = self.current_body() {
            find_buffer_matches(body.lines(), &query, self.search.buffer_search.case_sensitive)
        } else {
            return;
        };
        self.search.buffer_search.matches = matches;
    }

    pub fn scroll_to_current_match(&mut self) {
        if let Some(m) = self.search.buffer_search.current_match() {
            ...
            if self.editor.mode == Mode::Edit {
                ...
                for (idx, source_line) in self.document.content_items.iter().map(ContentItem::source_line).enumerate() {
                    if source_line >= target_row {
                        ...
                        if idx > half_height {
                            self.document.content_scroll_offset = idx - half_height;
                        } else {
    ...
    pub fn buffer_search_next(&mut self) {
        self.search.buffer_search.next_match();
        self.scroll_to_current_match();
    }

    pub fn buffer_search_prev(&mut self) {
        self.search.buffer_search.prev_match();
        self.scroll_to_current_match();
    }
    ...
    pub fn get_visible_sidebar_indices(&self) -> Vec<usize> {
        if self.search.search_active && !self.search.search_query.is_empty() {
            ...
            (0..self.vault.sidebar_items.len()).collect()
        }
    }
    ...
    pub fn next_outline(&mut self) {
        ...
        let i = match self.document.outline_state.selected() {
            Some(i) => (i + 1) % self.document.outline.len(),
            None => 0,
        };
        self.document.outline_state.select(Some(i));
    }
    ...
    pub fn goto_last_outline(&mut self) {
        if !self.document.outline.is_empty() {
            self.document.outline_state.select(Some(self.document.outline.len() - 1));
        }
    }

    pub fn jump_to_outline(&mut self) {
        if let Some(selected) = self.document.outline_state.selected() {
            if let Some(outline_item) = self.document.outline.get(selected) {
                let target_line = outline_item.line;
                if target_line < self.document.content_items.len() {
                    self.unfold_heading_at(target_line);
                    self.document.content_cursor = target_line;
                }
                self.state.focus = Focus::Content;
            }
        }
    ...
    pub fn current_note(&self) -> Option<&Note> {
        self.vault.notes.get(self.vault.selected_note)
    }
    ...
    pub fn show_toast(&mut self, message: impl Into<String>, kind: ToastKind) {
        self.state.toast = Some(Toast { message: message.into(), kind, shown_at: self.dependencies.clock.now() });
    }
    ...
    pub fn tick_toast(&mut self) -> bool {
        ...
        if self.state.toast.as_ref().is_some_and(|t| t.is_expired_at(now)) {
    ...
    for (row, line) in lines.enumerate() {
        ...
        for (col, (byte_start, _)) in search_line.char_indices().enumerate() {
            if search_line[byte_start..].starts_with(query) {
                matches.push(BufferSearchMatch { row, start_col: col, end_col: col + query_len });
            }
        }
    }
    matches
