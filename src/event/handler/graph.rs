...
pub(super) fn graph_fit_zoom(app: &App) -> f32 {
    let Some((min_x, min_y, max_x, max_y)) = graph_bounds(app) else {
        return ekphos_graph::fit_zoom_for_bounds(1.0, 1.0, app.graph_view.view_width, app.graph_view.view_height);
    };
    let graph_width = (max_x - min_x).max(3.0);
    let graph_height = (max_y - min_y).max(2.0);
    ekphos_graph::fit_zoom_for_bounds(graph_width, graph_height, app.graph_view.view_width, app.graph_view.view_height)
}
...
pub(super) fn center_graph_bounds(app: &mut App) {
    ...
    app.graph_view.viewport_x = center_x - app.graph_view.view_width / app.graph_view.zoom / 2.0;
    app.graph_view.viewport_y = center_y - app.graph_view.view_height / app.graph_view.zoom / 2.0;
...
pub(super) fn handle_graph_view_dialog(app: &mut App, key: crossterm::event::KeyEvent) {
    ...
    match key.code {
        ...
        KeyCode::Char('0') => {
            ...
            app.graph_view.zoom = 1.0f32.clamp(fit_zoom, ekphos_graph::GRAPH_MAX_ZOOM);
...
pub(super) fn open_selected_graph_node(app: &mut App) {
    let Some(node_idx) = app.graph_view.selected_node else {
        return;
    };
    let Some(node) = app.graph_view.nodes.get(node_idx) else {
        return;
    };
...
pub(super) fn select_graph_node(app: &mut App, idx: usize, center: bool) {
    if let Some(note_id) = app.graph_view.nodes.get(idx).map(|node| node.note_id) {
        app.graph_view.selected_node = Some(idx);
        app.graph_view.selected_note_index = app.note_index_for_id(note_id);
        if center {
            center_on_selected_node(app);
        }
    }
...
pub(super) fn cycle_graph_match(app: &mut App, delta: isize) {
    let skip_context_root = app.graph_view.mode == ekphos_graph::GraphMode::Local && !app.graph_view.filter_query.trim().is_empty();
    ...
    let next = app
        ...
        .map(|position| (position as isize + delta).rem_euclid(candidates.len() as isize) as usize)
