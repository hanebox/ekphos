    ...
    F: Fn(&str) -> bool,
{
    ...
    let styled_line = if line.starts_with("###### ") {
        ...
        Line::from(vec![
            ...
            Span::styled(
                line.trim_start_matches("##### "),
                Style::default().fg(content_theme.heading4).add_modifier(Modifier::BOLD),
            ),
        ])
    } else if line.starts_with("#### ") {
        ...
        Line::from(vec![
            ...
            Span::styled(
                line.trim_start_matches("### "),
                Style::default().fg(content_theme.heading3).add_modifier(Modifier::BOLD),
            ),
        ])
    } else if line.starts_with("## ") {
        ...
        Line::from(vec![
            ...
            Span::styled(
                line.trim_start_matches("## "),
                Style::default().fg(content_theme.heading2).add_modifier(Modifier::BOLD),
            ),
        ])
    } else if line.starts_with("# ") {
        ...
        Line::from(vec![
            ...
            Span::styled(
                ...
                Style::default().fg(content_theme.heading1).add_modifier(Modifier::BOLD),
            ),
        ])
    } else if line.starts_with("- ") {
        // Bullet list
...
pub(super) fn render_inline_image_with_cursor(
    f: &mut Frame,
    app: &mut App,
    item_index: usize,
    path: DocumentRange,
...
pub(super) fn render_inline_thumbnails(
    f: &mut Frame,
    app: &mut App,
    item_index: usize,
    area: Rect,
...
pub(super) fn render_details(
    ...
    let summary_spans = vec![
        ...
        Span::styled(expanded_summary, Style::default().fg(theme.info).add_modifier(Modifier::BOLD)),
    ];
...
pub(super) fn render_tag_badges_inline(f: &mut Frame, theme: &Theme, tags: &[Box<str>], date: Option<&str>, area: Rect, is_cursor: bool) {
    ...
    for (i, tag) in tags.iter().enumerate() {
        ...
        spans.push(Span::styled(
            format!(" {} ", tag),
            Style::default().fg(theme.content.tag).bg(theme.content.tag_background),
        ));
    }
    if let Some(d) = date {
        if !tags.is_empty() {
