...
use crate::app::{SearchPickerMode, SearchPickerState};
...
pub struct SearchPickerView<'a> {
    pub theme: &'a Theme,
    pub keymap: &'a Keymap,
    pub picker: &'a SearchPickerState,
}

pub struct SearchPickerRender {
    pub area: Rect,
    pub results_area: Rect,
}

pub fn render_search_picker(f: &mut Frame, view: SearchPickerView<'_>) -> Option<SearchPickerRender> {
    let SearchPickerState::Open { mode, query, file_results, content_results, hydrated_content_results, content_preview, selected_index, scroll_offset, search_in_progress, .. } = view.picker else {
        return None;
    };
    ...
    let (results_len, content_height) = match mode {
        SearchPickerMode::Files => {
            ...
            let height: usize = file_results.iter().skip(*scroll_offset).take(visible_items).map(|r| if r.folder_hint.is_some() { 2 } else { 1 }).sum();
            (file_results.len(), height.max(POPUP_MIN_CONTENT_HEIGHT))
        }
        SearchPickerMode::Content => (content_results.len(), POPUP_MIN_CONTENT_HEIGHT_WITH_PREVIEW),
    };
    ...
    let popup_block = Block::default()
        .title(format!(" Search ({}) ", view.keymap.binding_label(AppCommand::OpenQuickSearch)))
        .title_bottom(
    ...
    let inner_area = Rect::new(popup_area.x + 1, popup_area.y + 1, popup_area.width.saturating_sub(2), popup_area.height.saturating_sub(2));
    let mut header_lines: Vec<Line> = Vec::new();
    header_lines.push(Line::from(""));
    let files_style = if *mode == SearchPickerMode::Files { Style::default().fg(theme.primary).add_modifier(Modifier::BOLD) } else { Style::default().fg(theme.muted) };
    let content_style = if *mode == SearchPickerMode::Content { Style::default().fg(theme.primary).add_modifier(Modifier::BOLD) } else { Style::default().fg(theme.muted) };
    header_lines.push(Line::from(vec![Span::raw(" "), Span::styled("← ", Style::default().fg(theme.muted)), Span::styled("Files", files_style), Span::styled(" | ", Style::default().fg(theme.muted)), Span::styled("Content", content_style), Span::styled(" →", Style::default().fg(theme.muted))]));
    header_lines.push(Line::from(""));
    let placeholder = if *mode == SearchPickerMode::Files { "Search notes..." } else { "Search content..." };
    let input_line =
        if query.is_empty() { Line::from(vec![Span::raw(" "), Span::styled(placeholder, Style::default().fg(theme.muted))]) } else { Line::from(vec![Span::raw(" "), Span::styled(query.clone(), Style::default().fg(theme.foreground)), Span::styled("█", Style::default().fg(theme.primary))]) };
    header_lines.push(input_line);
    header_lines.push(Line::from(Span::styled("─".repeat(inner_area.width as usize), Style::default().fg(theme.muted))));
    ...
    let results_area = Rect::new(inner_area.x, inner_area.y + header_height, inner_area.width, inner_area.height.saturating_sub(header_height));
    if has_preview {
        let list_area = Rect::new(results_area.x, results_area.y, list_width.saturating_sub(1), results_area.height);
        let preview_area = Rect::new(results_area.x + list_width, results_area.y, results_area.width.saturating_sub(list_width), results_area.height);
        let sep_area = Rect::new(results_area.x + list_width.saturating_sub(1), results_area.y, 1, results_area.height);
        let sep_lines: Vec<Line> = (0..sep_area.height).map(|_| Line::from(Span::styled("│", Style::default().fg(theme.muted)))).collect();
        ...
        if content_results.is_empty() {
            ...
            render_content_results_compact(&mut list_lines, hydrated_content_results, *selected_index, *scroll_offset, list_area.width, theme, query);
        }
        let list = Paragraph::new(list_lines).style(Style::default().bg(theme.background_secondary));
        f.render_widget(list, list_area);
        render_preview(f, hydrated_content_results.iter().find(|entry| entry.result_index == *selected_index), content_preview.as_ref(), preview_area, query, theme, *search_in_progress);
        Some(SearchPickerRender { area: popup_area, results_area: list_area })
    } else {
        ...
        match mode {
            SearchPickerMode::Files => {
                if file_results.is_empty() {
                    result_lines.push(Line::from(vec![Span::raw(" "), Span::styled(if query.is_empty() { "Type to search files..." } else { "No matching files" }, Style::default().fg(theme.muted))]));
                } else {
            ...
            SearchPickerMode::Content => {
                if *search_in_progress {
                    ...
                    result_lines.push(Line::from(vec![Span::raw(" "), Span::styled(if query.is_empty() { "Type to search content..." } else { "No matching content" }, Style::default().fg(theme.muted))]));
                } else {
                    render_content_results(&mut result_lines, hydrated_content_results, *selected_index, *scroll_offset, max_name_width, results_area.width, theme);
                }
            }
        }
        ...
        Some(SearchPickerRender { area: popup_area, results_area })
    }
}
fn render_file_results(lines: &mut Vec<Line>, results: &[crate::app::FilePickerResult], selected_index: usize, scroll_offset: usize, max_name_width: usize, popup_width: u16, theme: &crate::config::Theme) {
    for (idx, result) in results.iter().enumerate().skip(scroll_offset).take(POPUP_MAX_VISIBLE_ITEMS) {
        ...
        let style = if is_selected { Style::default().fg(theme.background).bg(theme.primary).add_modifier(Modifier::BOLD) } else { Style::default().fg(theme.foreground) };
        if is_selected {
            ...
            lines.push(Line::from(vec![Span::styled(" ".to_string(), style), Span::styled(display_name, style), Span::styled(padding_right, style)]));
        } else {
            lines.push(Line::from(vec![Span::raw(" "), Span::styled(display_name, style)]));
        }
        if let Some(ref folder) = result.folder_hint {
            let hint_style = if is_selected { Style::default().fg(theme.muted).bg(theme.primary) } else { Style::default().fg(theme.muted) };
            ...
            if is_selected {
                ...
                lines.push(Line::from(vec![Span::styled(hint_text, hint_style), Span::styled(padding_right, Style::default().bg(theme.primary))]));
            } else {
...
fn render_content_results(lines: &mut Vec<Line>, results: &[crate::app::HydratedSearchResult], selected_index: usize, scroll_offset: usize, max_name_width: usize, popup_width: u16, theme: &crate::config::Theme) {
    for entry in results.iter().filter(|entry| entry.result_index >= scroll_offset).take(POPUP_MAX_VISIBLE_ITEMS_CONTENT) {
        ...
        let title_style = if is_selected { Style::default().fg(theme.background).bg(theme.primary).add_modifier(Modifier::BOLD) } else { Style::default().fg(theme.foreground) };
        let line_hint_style = if is_selected { Style::default().fg(theme.muted).bg(theme.primary) } else { Style::default().fg(theme.muted) };
        if is_selected {
            let used_width = 1 + display_name.chars().count() + line_hint.len();
            let padding = " ".repeat(content_width.saturating_sub(used_width));
            lines.push(Line::from(vec![Span::styled(" ".to_string(), title_style), Span::styled(display_name, title_style), Span::styled(padding, title_style), Span::styled(line_hint, line_hint_style)]));
        } else {
            let used_width = 1 + display_name.chars().count() + line_hint.len();
            let padding = " ".repeat(content_width.saturating_sub(used_width));
            lines.push(Line::from(vec![Span::raw(" "), Span::styled(display_name, title_style), Span::raw(padding), Span::styled(line_hint, line_hint_style)]));
        }
        ...
        let match_style = if is_selected { Style::default().fg(theme.warning).bg(theme.primary).add_modifier(Modifier::BOLD) } else { Style::default().fg(theme.warning).add_modifier(Modifier::BOLD) };
        let normal_style = if is_selected { Style::default().fg(theme.muted).bg(theme.primary) } else { Style::default().fg(theme.muted) };
        ...
        if is_selected {
            ...
            let padding_right = " ".repeat(content_width.saturating_sub(used_width));
            lines.push(Line::from(vec![Span::styled("  ", normal_style), Span::styled(before, normal_style), Span::styled(matched, match_style), Span::styled(after, normal_style), Span::styled(padding_right, normal_style)]));
        } else {
            lines.push(Line::from(vec![Span::raw("  "), Span::styled(before, normal_style), Span::styled(matched, match_style), Span::styled(after, normal_style)]));
        }
        if let Some(ref folder) = result.folder_hint {
            let hint_style = if is_selected { Style::default().fg(theme.muted).bg(theme.primary) } else { Style::default().fg(theme.muted) };
            let hint_text = if folder.chars().count() > max_name_width.saturating_sub(2) {
...
fn render_content_results_compact(lines: &mut Vec<Line>, results: &[crate::app::HydratedSearchResult], selected_index: usize, scroll_offset: usize, area_width: u16, theme: &crate::config::Theme, query: &str) {
    ...
    for entry in results.iter().filter(|entry| entry.result_index >= scroll_offset).take(POPUP_MAX_VISIBLE_ITEMS_CONTENT) {
        ...
        let prefix_style = if is_selected { Style::default().fg(theme.muted).bg(theme.primary) } else { Style::default().fg(theme.muted) };
        let normal_style = if is_selected { Style::default().fg(theme.background).bg(theme.primary) } else { Style::default().fg(theme.foreground) };
        let highlight_style = if is_selected { Style::default().fg(theme.warning).bg(theme.primary).add_modifier(Modifier::BOLD) } else { Style::default().fg(theme.warning).add_modifier(Modifier::BOLD) };
...
fn render_preview(f: &mut Frame, selected: Option<&crate::app::HydratedSearchResult>, preview: Option<&crate::app::ContentSearchPreview>, area: Rect, query: &str, theme: &crate::config::Theme, search_in_progress: bool) {
    let header_height = 2u16;
    if area.height <= header_height {
        return;
    }
    let header_area = Rect { x: area.x, y: area.y, width: area.width, height: header_height };
    let content_area = Rect { x: area.x, y: area.y + header_height, width: area.width, height: area.height - header_height };
    let query_lower = query.to_lowercase();
    if let Some(result) = selected.map(|entry| &entry.result) {
        let match_line = result.line_number.saturating_sub(1); // Convert to 0-indexed
        let file_header = if let Some(ref folder) = result.folder_hint { format!("{}/{}", folder, result.display_name) } else { result.display_name.clone() };
        ...
        let header_lines = vec![Line::from(vec![Span::styled(" ", Style::default()), Span::styled(display_header, Style::default().fg(theme.primary).add_modifier(Modifier::BOLD))]), Line::from(Span::styled(" ".repeat(area.width as usize), Style::default().fg(theme.muted)))];
        let header = Paragraph::new(header_lines).style(Style::default().bg(theme.background_secondary));
        f.render_widget(header, header_area);
        ...
        if let Some(preview) = preview {
            ...
            for (offset, line_content) in preview.lines.iter().enumerate() {
                ...
                let line_num_style = if is_match_line { Style::default().fg(theme.primary).add_modifier(Modifier::BOLD) } else { Style::default().fg(theme.muted) };
                ...
                for (seg_idx, segment) in wrapped_segments.iter().enumerate() {
                    let prefix = if seg_idx == 0 { format!("{:>4} │ ", display_line_num) } else { "     │ ".to_string() };
        ...
        let content = Paragraph::new(content_lines).style(Style::default().bg(theme.background_secondary)).scroll((scroll_offset as u16, 0));
        ...
        let header_area = Rect { x: area.x, y: area.y, width: area.width, height: 2.min(area.height) };
        let content_area = Rect { x: area.x, y: area.y + header_area.height, width: area.width, height: area.height.saturating_sub(header_area.height) };
        let header_lines = vec![Line::from(Span::styled(" Preview", Style::default().fg(theme.muted))), Line::from(Span::styled(" ".repeat(area.width as usize), Style::default().fg(theme.muted)))];
        let header = Paragraph::new(header_lines).style(Style::default().bg(theme.background_secondary));
        f.render_widget(header, header_area);
