        ...
        ekphos_core::markdown::wiki_links(text)
            ...
            .map(|link| {
                ...
                WikiLinkInfo {
                    ...
                    heading: link.heading.map(str::to_string),
                    display_text: link.alias.map(str::to_string),
                    start_col: rendered_start,
                    end_col: rendered_end,
                    is_valid: self.wiki_link_exists(link.target),
                }
            })
            .collect()
    }
        ...
        if let Some(note) = self.notes.get(note_idx) {
            if let Some(ref file_path) = note.file_path {
                ...
                while let Some(parent) = current {
                    ...
                    if !self.folder_states.get(&parent.to_path_buf()).copied().unwrap_or(expanded_by_default) {
                        self.folder_states.insert(parent.to_path_buf(), true);
                        needs_rebuild = true;
                    }
                    if is_root {
            ...
            if let Some(wiki_path) = self.get_wiki_path_for_note(idx) {
                ...
                if let Some(score) = fuzzy_match(&note.title, note_query) {
                    ...
                    suggestions.push(WikiSuggestion {
                        ...
                        is_folder: false,
                        path: note.file_path.as_ref().map(|p| p.display().to_string()).unwrap_or_default(),
                        score,
                        folder_hint,
                    });
                }
