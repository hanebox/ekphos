...
impl DocumentRange {
    pub fn new(start: usize, end: usize) -> Self {
        ...
        Self {
            start: u32::try_from(start).expect("active document exceeds 4 GiB"),
            end: u32::try_from(end).expect("active document exceeds 4 GiB"),
        }
    }
...
impl DocumentSnapshot {
    pub fn new(body: Arc<str>) -> Self {
        ...
        Self {
            body,
            line_offsets: offsets.into_boxed_slice(),
        }
    }
    ...
    pub fn range_within_line(&self, line: usize, range: Range<usize>) -> Option<DocumentRange> {
        ...
        (range.start <= range.end && range.end <= line_range.len())
            .then(|| DocumentRange::new(line_range.start() + range.start, line_range.start() + range.end))
    }
